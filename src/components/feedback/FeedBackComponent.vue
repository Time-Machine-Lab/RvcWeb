...
// import {getLoginUserInfo} from "@/api/user/userApi.ts";
// import {useUserStore} from "@/view/user/info/userStore.ts";
// import {Profile} from "@/api/user/userTypes.ts";
// import {storage} from "@/utils/storage.ts";
// 获取的是帖子的fbid
const { data } = defineProps(['data']);
const FeedbackData = ref<FeedbackItem>(<FeedbackItem>{})
const Comment = ref<Comment>({page: "1", limit: "6",total:"",pageList:[]})
...
const load = () => {
  ...
  setTimeout(() => {
    ...
    if (loadedCommentsCount < total) {
      getComment(data, nextPage, parseInt(Comment.value.limit), "up_num")
          .then((res: any) => {
            console.log(res);
            ...
            // 如果已加载数量加上当前页数量大于或等于总数量，说明已加载完所有评论，可以停止加载
            if (loadedCommentsCount + res.data.pageList.length >= total) {
              console.log('已加载完所有评论');
              // 这里可以触发某个事件或设置一个标志来表示已加载完所有评论
            }
          })
          .catch((error: any) => {
...
//   fbid: FeedbackData.value.fbid,
//   title: FeedbackData.value.title,
        ...
        <div class="box-contain-left">
          ...
          <div class="left-login flex">
            ...
            <router-link to="/login" target="_blank" style="color:#3a3a3a;margin-right: 10px" class="flex">
              <p style="margin-right:5px">使用RVC帐户登录</p>
...
<!--                  <div v-if="item.hasShow" style="position:relative">-->
                  <div style="position:relative">
                    ...
                    <div class="left-review-reply flex">
                      ...
                      <button @click="reply(item.replyCmId)" class="flex">
